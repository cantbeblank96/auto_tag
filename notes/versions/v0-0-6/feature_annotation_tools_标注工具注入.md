...
> 通用「工具注入」框架：标注时按需运行客观测量工具（kevin_sdk），把结果以**纯文本 JSON** 注入 VLM prompt，
...
```
prompt 文本 → [Reference example] 样图段 → [Tool measurement] 工具段 → 收尾提醒 → 待标注图（最后）
```
...
- `auto_tag/backend/routers/annotation_tools.py`：
  ...
  - `POST /api/annotation_tools/analyze`：精简问题定义 + 工具描述送 VLM（max_tokens=16384，
    防 thinking 模型 reasoning 截断），返回 `{suggestions: {question_key: [tools]}}`。
...
- 三工具真实执行（image_158.jpg）：detect score=0.866 height_ratio=0.45、pose yaw=-22.8/roll=88.1（图旋转 90°）、
  attribute Gender=Male Age=33.8；
...
- Web E2E：绑定 tools 后提交单图任务，日志确认「标注工具注入: face_detect, head_pose, face_attribute」，
  最终标签 face_size=1（测量 0.45∈[25%,70%]）、head_pose=slight_yaw（结合图片正确解读旋转图）、gender=male，与测量一致；
