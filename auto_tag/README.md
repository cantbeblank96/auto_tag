...
* **双阈值无监督聚类（CLIP 建簇）+ 异步 VLM 池：**
  * `d ≤ τ_dup` (极度相似): 判定为视频冗余相邻帧，侧车登记，不额外入库。
  * `τ_dup < d ≤ τ_cls` (语义相似/同场景): 并入同簇，继承簇中心标签，不调 VLM。
  * `d > τ_cls` (全新场景): 立刻建簇中心并入队；全局 `vlm_concurrency` 个 worker 并行打标后回写。
* **增量式持久化：** 数据向量和 Json 标签通过 ChromaDB 持久化，支持随开随用，无缝增量处理新的批次。
* **接口容灾：** VLM 多模型 Failover / Round-Robin、熔断器、Schema 校验与对话式改正、Exponential Backoff。
* **可观测性：** 可选 `pipeline_debug` 写出 VLM 时序图与 HTTP trace；Web 任务页展示耗时与双进度条。
